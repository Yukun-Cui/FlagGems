...
# Tolerances measured against the aten op, oracle fed the same rounded values
# the kernel sees, over these shapes x 8 seeds (worst observed max-abs-err):
#   fp32     1.0e-06  -> 1e-4
#   fp16     9.8e-04  -> 4e-3
#   bfloat16 7.7e-03  -> 2e-2
# fp32 sits ~100x under its budget: the kernel reproduces FBGEMM's uint8
# activation quantization and int32 accumulation, so the only residual is the
...
def test_quantized_gru_cell(shape, dtype):
    ...
    utils.gems_assert_close(
        res_out.float().cpu(), ref_out, torch.float32, atol=_ATOL[dtype]
    )
